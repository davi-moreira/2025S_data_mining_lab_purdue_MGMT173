...
- **Assessment**: Checks for non-linearity and unequal error variances.
...
- **Assessment**: Checks if residuals are normally distributed.
...
- **Observation**: Provides information on the overall fit and significance of the model.
...
```{r echo=TRUE, eval=FALSE, results='hide', warning=FALSE, message=FALSE}
# Residuals vs Fitted plot
plot(model_mult$fitted.values, model_mult$residuals,
     main = "Residuals vs Fitted",
     xlab = "Fitted Values",
     ylab = "Residuals",
     pch = 19, col = "darkgreen")
...
```{r echo=TRUE, eval=FALSE, results='hide', warning=FALSE, message=FALSE}
# Q-Q plot for residuals
qqnorm(model_mult$residuals)
qqline(model_mult$residuals, col = "red")
```
...
```{r}
# Scale-Location plot
plot(model_mult$fitted.values, sqrt(abs(model_mult$residuals)),
     main = "Scale-Location",
     xlab = "Fitted Values",
...
- **Observation**: VIF values greater than 5 or 10 indicate multicollinearity.


## Key Takeaways

- Multiple regression allows for the inclusion of several predictors to improve model accuracy.
- Important to check for multicollinearity and remove redundant variables.
...
- Apply the model to new datasets for prediction.
